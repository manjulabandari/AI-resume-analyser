...
# Company skill match constants and helpers
COMPANY_PROFILES = {
    "Google": {
        "title_keywords": ["software engineer", "distributed systems", "python", "c++", "docker", "kubernetes"],
        "must_have_skills": ["python", "c++", "distributed", "systems"],
        "optional_skills": ["tensorflow", "machine learning", "gcp", "ai"]
    },
    "Amazon": {
        "title_keywords": ["software development engineer", "sd", "java", "aws", "microservices"],
        "must_have_skills": ["aws", "java", "microservices"],
        "optional_skills": ["kubernetes", "python", "data", "lambda"]
    },
    "Microsoft": {
        "title_keywords": ["software engineer", "azure", "c#", "dotnet", "cloud"],
        "must_have_skills": ["azure", "c#", "dotnet"],
        "optional_skills": ["sql", "kubernetes", "devops"]
    }
}
...
def match_candidate_to_company(resume_text, company_profile):
    text = normalize_text(resume_text)
    tokens = set(text.split())
    score = 0
    max_score = 0

    for skill in company_profile.get("must_have_skills", []):
        max_score += 5
        if skill.lower() in tokens:
            score += 5

    for skill in company_profile.get("optional_skills", []):
        max_score += 2
        if skill.lower() in tokens:
            score += 2

    for kw in company_profile.get("title_keywords", []):
        max_score += 1
        if kw.lower() in text:
            score += 1

    fit_percentage = round((score / max_score) * 100, 1) if max_score else 0.0
    return fit_percentage


def company_match_results(resume_text):
    results = []
    for company, profile in COMPANY_PROFILES.items():
        fit = match_candidate_to_company(resume_text, profile)
        status = "Strong" if fit >= 70 else "Good" if fit >= 50 else "Needs Improvement"
        results.append({
            "company": company,
            "fit_percentage": fit,
            "status": status
        })
    return sorted(results, key=lambda x: x["fit_percentage"], reverse=True)
...
class ResumeAnalyzer:
    ...
    def analyze(self, resume_text, job_description=None):
        ...
        try:
            ...
            # Add company-specific matching results
            analysis['company_matches'] = company_match_results(resume_text)
        ...
        # Include raw text for optional detailed contact output in report
    ...
    def generate_report(self, analysis):
        ...
        # Header
        parts = ["PROFESSIONAL RESUME ANALYSIS REPORT", "=" * 42]
        parts.append(f"Overall Score: {analysis.get('overall_score', 0)}/100")
        parts.append(f"Status: {analysis.get('summary', {}).get('status', 'Unavailable')}")

        # Job match summary
        parts.append("\n1) JOB MATCH SUMMARY")
        if analysis.get('job_match'):
            job_match = analysis['job_match']
            parts.append(f" - Match Percentage: {job_match.get('match_percentage', 0)}%")
            parts.append(f" - Matched Technical Skills: {', '.join(job_match.get('matched_tech_skills', [])) or 'None'}")
            parts.append(f" - Missing Technical Skills: {', '.join(job_match.get('missing_tech_skills', [])) or 'None'}")
        else:
            parts.append(" - No job description provided or job match data unavailable")
        ...
        if strengths:
            ...
            parts.append(" - No strengths identified")
        ...
        if improvements:
            ...
            parts.append(" - No critical improvements identified")
        ...
        parts.append(f" - Word count: {stats.get('word_count', 0)}")
        parts.append(f" - Technical skills found: {stats.get('technical_skills_count', 0)}")
        parts.append(f" - Action verbs found: {stats.get('action_verbs_count', 0)}")
        parts.append(f" - Metrics found: {stats.get('metrics_count', 0)}")

        # Optional contact and sections
        contact = self.extract_contact_info(analysis.get('raw_text', '')) if analysis.get('raw_text') else {}
        if contact:
            parts.append("\n6) CONTACT DATA")
            parts.append(f" - Email: {contact.get('email', 'N/A')}")
            parts.append(f" - Phone: {contact.get('phone', 'N/A')}")
            parts.append(f" - LinkedIn: {contact.get('linkedin', 'N/A')}")

        # Company match section
        company_matches = analysis.get('company_matches', [])
        if company_matches:
            parts.append("\n7) COMPANY FIT RECOMMENDATIONS")
            for cm in company_matches[:5]:
                parts.append(f" - {cm['company']}: {cm['fit_percentage']}% ({cm['status']})")

        # Advice note
        parts.append("\nNote: Use this report to quickly evaluate fit and refine the resume for recruiter screening.")
