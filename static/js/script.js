class ResumeAnalyzerApp {
    ...
    displayResults(analysis) {
        ...
        // Update company match results
        this.updateCompanyMatch(analysis);
    ...
    updateCompanyMatch(analysis) {
        const container = document.getElementById('companyMatchGrid');
        if (!container) return;

        const matches = (analysis.company_matches || []).slice(0, 6);
        if (matches.length === 0) {
            container.innerHTML = '<p class="text-muted">No company matches available</p>';
            return;
        }

        container.innerHTML = matches.map(match => `
            <div class="stat-card">
                <h4>${this.escapeHtml(match.company)}</h4>
                <p><strong>${match.fit_percentage}%</strong> - ${this.escapeHtml(match.status)}</p>
            </div>
        `).join('');
    }
